...
%	\uestccover[<学院名称排版风格>]{<论文题目>}{<学科专业>}{<学号>}{<作者姓名>}{<指导教师>}{<教师职称>}{<学院>} % 单学位学士、研究生通用
	...
	% \forceblankpage % 当前英文摘要以单行（Keywords内容置于parbox中，逻辑上算一行）奇数页结束，印刷模式无法捕获此极端情况（与LaTeX的输出原理有关），所以未在此处插入偶数空白页。遇到这种情况，\forceblankpage命令用于在此处强制追加空白页。除此之外，不要随意使用该命令
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\textcolor{red}{当某一前置部分以单行奇数页结束时，印刷模式无法捕获该极端情况（与LaTeX的输出原理有关），无法正确插入偶数空白页（2025.05.01）。}\textbf{遇到这种情况，用户需要在该前置部分的结尾处使用\shadcmd{forceblankpage}命令，手动插入空白页，参见本文档\shad{英文摘要结尾处}的做法（默认为注释状态，以防误用）；或最后使用第三方PDF软件插入空白页（2025.05.25）。}
