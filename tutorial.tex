...
%	\uestccover[<学院名称排版风格>]{<论文题目>}{<学科专业>}{<学号>}{<作者姓名>}{<指导教师>}{<教师职称>}{<学院>} % 单学位学士、研究生通用
	...
	\null\par
	\begin{itemize}
		\item Overleaf用户可以一键搞定，无需额外操作。\textbf{\color{DarkRed}注意，Overleaf用户要在网站里选择TeXLive的2024版，亲测Overleaf目前的TeXLive2025版无法正确编译出“主要符号表”和“缩略词表”的页眉。本地用户只要保证TeXLive版本≥ 2024就不会有问题。}
	\end{itemize}
	...
	为了确保在Windows、MacOS、Overleaf等平台上编译出完全一致的结果，模板内置了所有用到的字体文件，这导致项目大小超出了Overleaf上传压缩包的限制。因此，Overleaf用户需要进行另一番操作：\textbf{先在Overleaf上新建一个空项目，然后解压本模板并拖拽文件和文件夹到新建的项目中}。\textcolor{DarkRed}{\textbf{切记网站里要选TeXLive2024}，经测试，线上目前的TeXLive2025反而无法编译出正确的“主要符号表”和“缩略词表”页眉，原因不明。}本地2024及以上的TeXLive均无任何问题！

	\textbf{提醒}：\textcolor{DarkRed}{Overleaf近期似乎收紧了普通账号的编译时间限制}，而模版需要完整编译4次才能建立缩略词表、交叉引用等信息，因此普通账号在Overleaf上会编译超时。所以：\textcolor{DarkRed}{1）要么，搭建自己的本地环境，最稳妥；2）要么，升级Overleaf的高级账号；3）要么，用第三方的Overleaf平台}，这里推荐一个：\href{https://www.onlinelatex.com/home/}{\color{DarkRed}\CJKunderline*[thickness=0.5bp, format=\color{DarkRed}]{OnlineLaTeX}}。如果你既想用Overleaf，又不想升级高级账号或是用第三方平台，那只能用由网友\href{https://github.com/note286}{\color{DarkRed}\CJKunderline*[thickness=0.5bp, format=\color{DarkRed}]{@note286}}提供的方法：\textbf{\color{DarkRed}在模版根目录新建名为\shad{latexmkrc}的文件，内容为\shad{\$max\_repeat = 1;}}。其原理是每次点击编译按钮后，让Overleaf只进行单次XeLaTeX编译，而不是默认的多次编译，这样只要单次编译时长不超过限制即可正常使用。代价是\textcolor{DarkRed}{需要用户手动编译3次生成“缩略词表”，4次建立交叉引用}。自此提醒添加之日起，模版发布将分为常规版和Overleaf特别版，\textbf{后者提供了\shad{latexmkrc}文件，仅针对想使用官方Overleaf又不想升级高级会员的用户}。（2026.01.03）
	...
	\begin{verbatim}
		...
		\RequirePackage{boxedminipage}  % 用于排版带边框的minipage环境
	\end{verbatim}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \shad{doctor}\textcolor{DarkRed}{〔默认〕} / \shad{prodoctor} / \shad{intdoctor} / \shad{ipdoctor} / \shad{master} / \shad{promaster} / \shad{intmaster} / \shad{ipmaster} / \shad{bachelor} / \shad{doublebachelor}：该选项设置学位论文类型，其与学位的对应关系如下：
			
			\begin{table}[!htb]
				\centering\hspace*{1.5cm}
				% \caption{各学位论文类型对应的选项设置} \label{tab: 学位论文类型选项}
				\begin{tabular}{p{9em} p{20em}}
					\toprule
					\textbf{选项值} & \textbf{对应学位类型} \\
					\midrule
					\shad{doctor} & 学术博士学位 \\
					\shad{prodoctor} & 专业博士学位 \\
					\shad{intdoctor} & International Doctor with Academic Degree \\
					\shad{ipdoctor} & International Doctor with Professional Degree \\
					\shad{master} & 学术硕士学位 \\
					\shad{promaster} & 专业硕士学位 \\
					\shad{intmaster} & International Master with Academic Degree \\
					\shad{ipmaster} & International Master with Professional Degree \\
					\shad{bachelor} & 学士学位 \\
					\shad{doublebachelor} & 双学士学位 \\
					\bottomrule
				\end{tabular}
			\end{table}
			...
			\item \shad{cmmmath}\textcolor{DarkRed}{〔默认〕} / \shad{timesmathnogreek} / \shad{timesmath}：该选项用于选择渲染公式使用的字体。其中，\shad{cmmmath}即对应LaTeX默认使用的Computer Modern Math，也是此类选项的默认值；\shad{timesmathnogreek}指定使用Times New Roman来渲染公式中的英文字母和数字，但不影响希腊字母、手写体和双线体；\shad{timesmath}则继续将希腊字母也设置成Times New Roman（个人觉得希腊字符用该字体有些违和），手写体和双线体仍保持原样。
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[width=0.6\linewidth]{huangrongguojing1}
		\caption{锁定仇人}
	\end{figure}
	...
	\begin{figure}[!h]
		\centering
		\subfloat[]{
			\includegraphics[width=0.4\linewidth]{yangguoxiaolongnv3}
			\label{fig: 见到姑姑嘻嘻}
		}
		\hfill
		\subfloat[]{
			\includegraphics[width=0.4\linewidth]{yangguoxiaolongnv6}
			\label{fig: 姑姑见我不嘻嘻}
		}
	...
	\begin{figure}[!htb]
		\centering
		\subfloat[]{
			\includegraphics[width=0.4\linewidth]{lve2}
			\label{fig: 绿萼2}
		}
		\hfill
		\subfloat[]{
			\includegraphics[width=0.4\linewidth]{yangguolve}
			\label{fig: 杨过绿萼}
		}
		\\
		\subfloat[]{
			\includegraphics[width=0.98\linewidth]{luwushuangchengying}
			\label{fig: GG}
		}
	...
	\begin{figure}[!htb]
		\centering
		\includegraphics[width=0.98\linewidth]{yangguoguojing}
		\caption{还是推主线吧，动手动手}
	\end{figure}
	...
	$\check{\boldsymbol{C}}^{"t}_n + \hat{p}^{"j}_{n, s} + \tilde{r}^{"t}_{u,b} + \overline{i}^t_{x, y} + \acute{\alpha}^t_v + \acute{\boldsymbol{\alpha}}^t + \hat{\boldsymbol{\alpha}}^t + f^t_j + "f^t_j + l^t_j + l^{"t}_j + \boldsymbol{\acute{\beta}}^t + \boldsymbol{\hat{\beta}}^t$
	...
	类似的，学士学位论文在引用\textbf{“电子文献”}时，“出版地”和“获取地址”只取其一、“发表更新日期”和“引用日期”同样只取其一，所以本科生只需要用\shad{address}域为之提供“出版地”\textcolor{DarkRed}{或}用\shad{url}域为之提供“获取地址”；用\shad{modifydate}域为之提供“发表更新日期”\textcolor{DarkRed}{或}“引用日期”即可。如下所示：
	\begin{verbatim}
	@digital{电子文献1,
	    author={Deverell, W and gler, D},
	    title={A companion to California history},
	...
	\begin{itemize}
		...
		从网站导出的原始bib数据可能缺失某些强制域，本模版会在编译后的参考文献列表中进行高亮提示，\textcolor{DarkRed}{请务必按照标注补充必要的bib信息}。（2026.04.22）
		
		\item 对中文参考文献，如果希望将它们的第四顺位及以后的作者显示为\shad{“等”}，则必须要在它们的bib条目中加入\shad{language=\{\}}域，并将值设置为\shad{schinese}。这是文献编译引擎判断该条参考文献是否是中文的唯一依据。类似的，\cite{罗杰斯2011}中的\shad{“等译”}、\shad{“2版”}、学士学位论文中某些文献类型日期中的“年月日”字样，均靠设置\shad{language=\{schinese\}}实现。\textbf{\textcolor{DarkRed}{我的建议是，虽然\shad{language}域并非强制，但对于中文文献，最好将其添加进去}}。
	...
	\begin{figure}[!b]
		\centering
		\includegraphics[width=0.4\linewidth]{hongqigong2}
	...
	\begin{figure}[!b]
		\centering
		\includegraphics[width=0.4\linewidth]{yangguo3}
		\captionsetup{list=no}% 阻止此图片显示在图目录中
	...
	\begin{figure}[!htb]
		\centering
		\includegraphics[width=0.45\linewidth]{hongqigong2}
		\captionsetup{list=no}% 阻止此图片显示在图目录中
		\caption{碰都不敢碰啊？小鬼}
	\end{figure}
	...
	\begin{figure}[!htb]
		\centering
		\includegraphics[width=0.45\linewidth]{yangguo3}
		\captionsetup{list=no}% 阻止此图片显示在图目录中
		\caption{难顶}
	\end{figure}
\end{document}