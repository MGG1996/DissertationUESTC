...
1. 本模板提供了印刷模式，该模式会根据学校的印刷规范自动在论文前置部分的必要位置插入空白页，大部分情况下无需用户人为处理编译后的文档，极大避免出错。
   *  :warning: **<font color=#8b0000>当某一前置部分以奇数页结束，且该页只有一行内容时，印刷模式无法正确追加空白页</font>**，这与LaTeX的输出原理有关（2025.05.01发现）。
    :bulb: :bulb: 若遇到这种极端情况，用户**需要在该前置部分的结尾处使用模板提供的`\forceblankpage`命令，手动插入空白页**；或在实际印刷前使用第三方PDF工具（如Adobe Acrobat）手动插入空白页。
